"""
Borrows from https://github.com/krrish94/nerf-pytorch/blob/master/nerf/cfgnode.py
...
def _merge_a_into_b(a: CfgNode, b: CfgNode, root: CfgNode, key_list: list):
    r"""Merge `CfgNode` `a` into `CfgNode` `sampled_pts`, clobbering the options in `sampled_pts` wherever they are also specified in `a`.
    """
    _assert_with_logging(
    ...
    _assert_with_logging(
        ...
        "`sampled_pts` (cur type {}) must be an instance of {}".format(type(b), CfgNode),
    )
